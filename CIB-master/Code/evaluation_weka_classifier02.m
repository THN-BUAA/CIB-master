function perf = evaluation_weka_classifier02( classifier,test)
...
if(Recall+Precision==0)
    F_measure=0;
    AGF = 0; % ¡¶Adjusted F-measure and kernel scaling for imbalanced data learning¡·
else
    F_measure=(2*Recall*Precision)/(Recall+Precision);
    
    F2 = (1+2^2)*(Recall*Precision)/(2^2*Precision+Recall);
    a = TP; TPtemp = FN; FNtemp = a;
    b = TN; TNtemp = FP; FPtemp = b;
    RecallTemp = TPtemp/(TPtemp+FNtemp);
    if(TPtemp+FPtemp==0)
        PrecisionTemp=0;
    else
        PrecisionTemp=TPtemp/(TPtemp+FPtemp);
    end
    if(RecallTemp+PrecisionTemp==0)
        AGF = 0;
    else
        invF05 = (1+0.5^2)*RecallTemp*PrecisionTemp/(0.5^2*PrecisionTemp+RecallTemp);
        AGF = sqrt(F2*invF05);
    end
...
perf = zeros(1,11);
perf(1) = Recall;perf(2)=Precision;perf(3)=F_measure; perf(4)=G_mean; perf(5)=AUC; perf(6)=Accuracy;perf(7)=PF;perf(8)=G_measure;perf(9)=MCC;perf(10)=Balance;perf(11)=AGF;
